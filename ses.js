...
function scrollArrived(el,offSet=300,windowHeightMultiplier=1){
	return el.getBoundingClientRect().top < offSet + windowHeightMultiplier*screen.availHeight;
}


function dyLazyLoadCode(DOMel, functionCallback){
	let myInterval = setInterval(function(){
		if(scrollArrived(DOMel)) {
			functionCallback();
			//console.log('Gatillar y parar');
			clearInterval(myInterval);
		}
		//else console.log('No gatillar');
	}, 250);
}
...
function resizeDyPod(){
  let dyPod=document.querySelectorAll('.dy-pod');
  let padreWidth = dyPod[0].parentElement.parentElement.offsetWidth;
  let dyColumnas = 5;
  if(padreWidth<1000){
    dyColumnas=4;
  }
  ...
  var varMargin = ((padreWidth/dyColumnas)-dyPod[0].offsetWidth)/2;
  dyPod.forEach(el=>{el.style.marginRight=varMargin+'px';el.style.marginLeft=varMargin+'px'});
}
...
function pintaProd(o,variables){
  ...
  const i = initializePod(o,elementsContainer,variables.omniture,variables.audiencia,variables.tipoBoton);
  
  dyPodBadge(variables.iconoEvento,variables.badgeClass,variables.badgeUrl,variables.badgeTitle,i.info);
  dyPodImage(o,i.info,variables.scene7);
  dyPodBrand(o,i.info);
  dyPodName(o,i.info);
  dyPodStars(o,i.info);
  dyPodPrecios(o,i.info,variables);
  if(variables.tipoBoton!=0){
    dyPodButton(o,i.btn,variables.tipoBoton,variables.labelBoton);
    
  }
}

/* Crea el HTML div del pod dentro de un <a> para hacerlo clickeabel. Devuelve el DOMElement del div pod*/
function initializePod(o,cont,omniture,audiencia,tipoBoton){
  let cid = "?cid="+omniture+"dy#"+audiencia
  ...
  if(tipoBoton==1){
    a.setAttribute('href',productUrl+ o.sku+cid);
  }
...
function dyPodBadge(iconoEvento,badgeClass,badgeUrl,badgeTitle,pod){
  if(iconoEvento){
    const badgeDiv = document.createElement('div');
    const badgePicture = document.createElement('picture');
    const badgeSource = document.createElement('source');
    const badgeImage = document.createElement('img');
    
    badgeDiv.classList.add(badgeClass);
    badgeSource.setAttribute('data-testid','source-image');
    badgeSource.setAttribute('srcset',badgeUrl);
    badgeImage.setAttribute('data-testid','scene7-image');
    badgeImage.setAttribute('alt',badgeTitle);
    badgeImage.setAttribute('title',badgeTitle);
    badgeImage.classList.add('float-right');
    badgeImage.classList.add('event-img');
    
    badgePicture.appendChild(badgeSource);
    badgePicture.appendChild(badgeImage);
    badgeDiv.appendChild(badgePicture);
    pod.appendChild(badgeDiv);
  }
}
...
function dyPodButton(o,pod,tipo,texto){
  ...
  if(tipo==3){
    btn.addEventListener("click",function () {
      dyBtnDisable();
      addToCartCatalyst(o.sku);
    }, false);
  }
  pod.appendChild(btn);
}
function dyBtnDisable(){
  document.querySelectorAll(".dy-pod-btn").forEach(function(element) {
    element.disabled = true;
    element.classList.add("disabledDy");
  });
}

function addToCartAtg(sku){
    const data = {
        formSubmissionData: [{
            productId: sku,
            quantity: 1,
    ...
    fetch('/rest/model/atg/commerce/order/purchase/CartModifierActor/addItemToBasket', {
        method: 'POST',
        headers: {
...
function addToCartCatalyst(sku){
  let url = '/s/checkout/v1/carts';
  let sessionId = sessionStorage.getItem('_dyjsession');
  let requestBody = 
    {"data":
      {"cart":
        {"cartLines":
      ...
      "metadata":
        {"zoneId":"1","priceGroup":"10"}
    };
    
  
  requestBody_string = JSON.stringify(requestBody);
  let cookiesArr = decodeURIComponent(document.cookie).split(';');
  let cookieCarIdVal2 = cookiesArr.filter(e=>{return e.trim().startsWith('cartId=')});
  if(cookieCarIdVal2.length>0){
    cookieCarIdVal2=cookieCarIdVal2[0].split('=')[1];
    url += '?cartId='+cookieCarIdVal2;
  }
  fetch(url, {
    method: 'POST',
    headers: {
    ...
    body: requestBody_string
  })
  .then(res => res.json())
  .then(res => {
    if (res.data) {
      document.cookie = 'cartId='+res.data.cart.cartId+';domain=homecenter.com.co;path=/';
      templateCustomAction(res.data);
    } else {
      console.log(res);
    }
  })
...
function dyPodPrecios(o,pod,variables){
  ...
  const iconos = {
    ...
    'AB':getAbIcon(),
    'combo':variables.svgCombo,
    'bundlePrice':variables.svgCombo,
    'internet':getInternetIcon(),
    'evento':variables.svgEvento,
  };
